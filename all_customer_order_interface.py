...
class POSView:
    ...
    def on_window_resize(self, event):
        ...
        if event.widget == self.root:
            ...
            elif width >= self.breakpoints["mobile"] and width < self.breakpoints[
                "tablet"] and self.current_layout != "tablet":
    ...
    def create_menu_items(self, items, categories):
        ...
        for i, category in enumerate(categories[:half_categories]):
            ...
            translated_cat = self.menu_model.get_translated_category(category, current_language) if hasattr(self,
                                                                                                            'menu_model') else category
        ...
        display_items = items if current_category == "All" else [item for item in items if
    ...
    def display_split_bills(self, split_bills):
        ...
        for i, (items, subtotal) in enumerate(split_bills, 1):
            ...
            if not items:  # Check if there are items
                ...
                ttk.Label(frame, text=f"{self.get_text('subtotal')}: $0.00",
                          font=('Arial', 12 if self.current_layout != "mobile" else 11, 'bold')).pack(anchor=tk.E,
                                                                                                      padx=10, pady=5)
                continue
            ...
            ttk.Label(frame, text=f"{self.get_text('subtotal')}: ${subtotal:.2f}",
                      font=('Arial', 12 if self.current_layout != "mobile" else 11, 'bold')).pack(anchor=tk.E, padx=10,
                                                                                                  pady=5)
        ...
        if self.current_layout == "mobile":
            ttk.Button(button_frame, text=self.get_text("print_bills"),
                       command=lambda: self.show_message(self.get_text("printing_bills"))).pack(fill=tk.X, padx=5,
                                                                                                pady=2)
...
class POSController:
    ...
    def checkout(self):
        ...
        self.view.show_message(
            f"{self.language_model.get_text('order')}{table_info}{group_info} {self.language_model.get_text('completed')}! {self.language_model.get_text('total')}: ${self.current_order.total:.2f}")
        self.clear_order()
